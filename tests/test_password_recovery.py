...
class TestPasswordRecovery:
    ...
    def test_forgot_password_page_opens(self, driver):
        ...
        allure.step("Проверить отображение страницы")
    ...
    def test_password_reset_form_submission(self, driver):
        allure.step("Открыть страницу восстановления")
        page = PasswordRecoveryPage(driver)
        page.open_forgot_password_page()
        allure.step("Заполнить и отправить форму")
        page.submit_restore_form(TestData.VALID_EMAIL)
        allure.step("Проверить появление поля ввода пароля")
    ...
    def test_password_visibility_toggle(self, driver):
        allure.step("Перейти на страницу смены пароля")
        ...
        allure.step("Проверить что пароль видимый")
