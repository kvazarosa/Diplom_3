...
class TestMainPage:
    ...
    def test_click_on_designer(self, driver):
        ...
        with allure.step("Переход в ленту заказов и возврат в конструктор"):
    ...
    def test_click_on_order_feed(self, driver):
        with allure.step("Инициализация главной страницы"):
            main_page = MainPage(driver)
        with allure.step("Клик на 'Лента заказов'"):
            main_page.go_to_order_feed()
    ...
    def test_ingredient_details_popup(self, driver):
        with allure.step("Инициализация главной страницы"):
            main_page = MainPage(driver)
        with allure.step("Клик на ингредиент булки"):
    ...
    def test_close_ingredient_popup(self, driver):
        with allure.step("Инициализация главной страницы"):
            main_page = MainPage(driver)
        with allure.step("Открытие и закрытие модального окна"):
            main_page.open_ingredient_details().close_modal()
        with allure.step("Проверка доступности конструктора"):
