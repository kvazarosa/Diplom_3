...
class MainPageLocators:
    ...
    # блок конструктора
    TEXT_ASSEMBLE_BURGER = (
        By.XPATH, "//h1[contains(@class, 'text_type_main-large') and contains(text(), 'Соберите бургер')]")
    ...
    DROP_ZONE = (
        By.XPATH,
        "//span[contains(@class, 'constructor-element__text') and contains(text(), 'Перетяните булочку сюда')]")
    CHECKOUT_BUTTON = (
        By.XPATH, "//button[contains(@class, 'button_type_primary') and contains(text(), 'Оформить заказ')]")
    CLEAR_CONSTRUCTOR_BUTTON = (By.XPATH, "//button[contains(text(), 'Очистить конструктор')]")

    # модальный блок
    INGREDIENT_DETAILS_MODAL = (
        By.XPATH, "//h2[contains(@class, 'modal__title') and contains(text(), 'Детали ингредиента')]")
    ...
    TEXT_BEGAN_TO_BE_PREPARED = (
        By.XPATH, "//p[contains(@class, 'main-small') and contains(text(), 'Ваш заказ начали готовить')]")
    ...
    # каунтер ингредиента
    INGREDIENT_COUNTER = (
        By.CSS_SELECTOR, "div.counter_counter__ZNLkj.counter_default__28sqi > p.counter_counter__num__3nue1")
